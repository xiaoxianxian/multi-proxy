...
**Step 6(MCP 资源面)**:`l2/mcp-server.js` `initialize()` 声明 `capabilities.resources:{listChanged:false}`;`listResources()`(活跃 agent 注册表)+ `readResource(uri)`(单 agent 只读视图,**密钥脱敏** `token/secret/credential/password/apiKey/bearer`→`[redacted]`);同步/异步 `handleMessage` 各补 `resources/list`+`resources/read`(未知 id 转 `-32602`);`isGateOpen()` 守门 gate 关被拒。`mcp-server.test.js` +7。
**数字(真跑)**:全量 jest **789/789(48 suites)**(`--runInBand`,含 mcp-server 33;= Step4 的 782 + 7 资源用例)· l2 demo **19/19 全 PASS**(18 旧 + 新 `l2-bootstrap` 13,**按退出码判定**——各 demo 收尾短语不一,grep "ALL PASS" 会误判 FAIL,以 exit code 为准)。**commit `e793ce5`**(5 文件:2 新 bootstrap + `mcp-server.js`+`routes/mcp.js`+`mcp-server.test.js`,375 insertions,`git add --` 显式)。`origin/main..HEAD` `0 3`=ahead 3(`2036480`Step4+`10140d0`归档+`e793ce5`)**未 push**,工作树 clean,等老板确认。
**铁律**:门控默认关(`PROXY_L2_MCP`/`PROXY_PROMPT_CACHE` off=零回归);热路径 `forward.js`/`codex-proxy/proxy.js` 0 触碰;`gateway.js:87` 维持 A;03 §7 待定项 / 真·上游 `cache_control` 注入维持 P2 待老板授权+锁 provider。详见 `MEMORY-2026-09-26.md §九` + `02 §二 Step 5/6` + `A3 #5`。

### §13.34 ③ L2 热路径改造 Step 4 落地(route() 纯决策化 / N3 防双执行 · 本地 commit 未 push · 09-26)

**改动(02 §二 Step 4 / N3)**:`route-engine.js:121-128`——非 shadow 且选中分支**删掉 `decision.execution = this._execute(...)`**,`route()` 回归**纯决策函数**;`action==='execute'` 仍标记(决策意图保留,零回归),但执行彻底交调用方(`_execute` / `runDag` / `callToolAsync` orchestrate)。根因(N3):原 `route()` 非 shadow 下 fire-and-forget 调 `_execute` 且 `decision.execution` 全仓 0 读取者 → 一旦有人按二期计划注入 executor,`route()` 先 fire 一次、调用方再 execute 一次 → **上游双调用**。实证:`.execution` 全仓 0 读取者;`runDag(:228)` 本就强制 shadow 规避双执行(独立执行路径),Step 4 不破坏;`route-engine-orchestrator.demo.js` 13/13 仍过。**测试(`route-engine.test.js` 新增 2 断言)**:① 非 shadow+注入 executor → `route()` 不触发 executor(`execCalls===0`)且不挂 `decision.execution`(`toBeUndefined`),执行仅由调用方 `_execute` 显式触发(`execCalls===1`, status done);② 非 shadow 多次 `route()` 始终 0 副作用(幂等决策)。**数字(真跑)**:`route-engine.test.js` 19/19(含新 2)·`route-engine-orchestrator.demo.js` 13/13·全量 jest **782/782**(`--runInBand`,比 §13.33 的 780 恰 +2=新断言,0 回归)·l2 demo 328/24 不变。**commit `2036480`**(仅 `route-engine.js`+`route-engine.test.js` 2 文件,`git add --` 显式;`origin/main..HEAD` `0 1`=ahead 1 **未 push**,工作树 clean,等老板确认)。**铁律**:热路径 `forward.js`/`codex-proxy/proxy.js` 0 触碰;Step 1-3 共享 registry / async 分派保持;Step 5(plugin bootstrap)/Step 6(MCP 资源面)因 adapter 为类非 PluginRuntime 形态有设计争议→先出方案待老板评审,不擅自动;03 §7 待定项 / 真注入 P2 维持待老板拍板/授权。
