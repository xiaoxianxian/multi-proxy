...
| **D5** | **DB model 名 ↔ fallbackChain 对齐 — override 的硬前置** | ✅ **已落地（2026-09-13，本地未 push）** | **（根因已被 ④ 实测坐实）** override 开时 `findProviderConfig` 查 `models` 表（**0 行**）→ fallback 首个 enabled=`kimi` → 路由到 kimi 却发引擎虚名 `deepseek-v4.1-flash`/`qwen3.8-flash`（上游不认）→ **生产 404**。 **修法（老板拍 ①，④→① 路径）**：④ 复用 `SecretsManager` 解密拉 3 家上游 `/v1/models` 实测真名（deepseek→`deepseek-flash`/`deepseek-v4-pro`；agnes→`agnes-2.5-flash`+11；kimi→`kimi-k2.6`/`kimi-k3`；ollama→`qwen3.8:27b-mlx`）；①/② 改 `DEFAULT_ROUTE_CONFIG` 虚名→真名（`deepseek-v4.1-flash`→`deepseek-v4-pro`，`qwen3.8-flash`→`qwen3.8:27b-mlx`，fallbackChain `[qwen3.8:27b-mlx, deepseek-v4-pro, agnes-2.5-flash, kimi-k2.6]`）+ `chatHandler.ts` 新增 `findProviderByType(modelName)`（模型名→`provider_id` 类型→首个启用，复用 `provider_id` 列，**不写 DB、不碰 `findProviderConfig` 热路径**），override 块改用它。门控默认 off，live 零影响。**改 `DEFAULT_ROUTE_CONFIG` 不改 live 路由**（live 走 `findProviderConfig`，两消费方 `routeShadow`/`override` 各自门控关）。同步 4 测试文件旧名断言，**124/124 绿（10 suites）+ tsc 净**。 **待办（需老板点头，写真实 DB）**：① 提交（只裹目标文件，绝不 `git add -A`）；② `enable` 原 disabled 的 `qwen3.8:27b-mlx` ollama provider（已实测 ollama 在跑 PID、无 H3/Ollama 并发、内存 22/48GB）；③ push。 **关键修正**：老板原 Q2=①「发引擎名」被 ④ 部分证伪——字面对 agnes 成立（三方同名），对 deepseek/kimi 需 `findProviderByType` 做「引擎名→类型」映射（见 `findProviderByType` 注释）；纯改 `provider.name` 匹配覆盖不了 deepseek/kimi（`provider.name` 是别名 `DeepSeek-Test`/`kimi`，非上游真名）。
| **D6** | **4d 健康信号接线 — D6-a 已做 / D6-b 卡 D4** | ✅/🔒 **D6-a 完成（2026-09-13，`461586d`，本地未 push）/ D6-b 卡 D4** | **D6-a 已落**：`chatHandler.ts` 新增 `loadEnabledProviderConfigs()`（与 `findProviderConfig` 共享「DB 行→ProviderConfig(含 key 解密)」构造，作 HealthMonitor 轮询+候选集构建单一数据源，**热路径零改动**）；`start.ts` 在 `PROXY_HEALTH_MONITOR` 门控下创建 `HealthMonitor`+构建 model→UUID 映射后 `setHealthContext` 注入，优雅退出 `stop()`；`routing-shadow.ts` 新增 `setHealthContext/clearHealthContext/buildHealthCandidates`，`computeShadowSuggestion` 用带健康候选集驱动 cost-optimization（未注入传 `undefined` 向后兼容）。映射固定 `healthy→ok / unhealthy→down / 未知→degraded / 死名→down`，新增 `health-integration.test.ts`(5 tests)，全量 **119/119 绿(9 suites)**、tsc 通过、热路径函数体零删除。仍 shadow 观测级、不改真实路由。**D6-b（健康决定真实路由）卡 D4 sign-off**——见方向四末尾草案。 |
