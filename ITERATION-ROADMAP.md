...
| **D5** | **DB model 名 ↔ fallbackChain 对齐 — override 的硬前置** | 🔴 **被 D4=C 抬高为阻塞** | **（2026-09-13 实测坐实）** 用探针测 override 真实行为：引擎建议 `deepseek-v4.1-flash`（coding）/`agnes-2.5-flash`（cheap/general），但 `models` 表 **0 行** → `findProviderConfig` 查不到引擎名 → fallback 首个 enabled provider=`kimi`(api.moonshot.cn) → **路由到 kimi 却发 `deepseek-v4.1-flash`，moonshot 不认 → 生产 404**。死名 `deepseek-v4.1-flash`(上游实 `deepseek-flash`/`deepseek-v4-pro`)+kimi 不在链， 在 round-robin 下不暴露(不看 model 名)，**override 一开就真 404**。 **D5 必须与 D4=C 同步或先行**：要么补 `models` 表对齐行(引擎名→provider UUID，FK=providers.id)， 要么改 `DEFAULT_ROUTE_CONFIG.fallbackChain` 死名为真实名。牵热路径+配置，需老板定映射方式后实施。
