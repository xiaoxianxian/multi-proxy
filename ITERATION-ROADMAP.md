...
| 阶段 | 内容 | 预估工作量 | 依赖 |
|------|------|-----------|------|
| **M1** | 方向一：DAG 健康依赖图 | 2-3 天 | 无 |
| **M2** | 方向二 2a：Provider 故障自动标记 | 1-2 天 | M1 |
| **M2** | 方向二 2b：跨 Proxy 故障感知 | 1 天 | M2-2a |
| **M3** | 方向二 2c：自动重启策略 | 1-2 天 | M2 |
| **M4** | 方向三 3a+3b：错误模式库 + 结构化日志 | 2-3 天 | 无（可并行） |
| **M5** | 方向三 3c：日志页面增强 | 1 天 | M4 |
| **M6** | 方向四：按任务类型智能派发（4a+4b+4c+4d） | 2-3 天 | 无（可与 M1+ 并行） |
...
1. **不破坏现有 564 个测试**：新增端点和逻辑独立于现有路由
...
*最后更新：2026-09-10（新增方向四：按任务类型智能派发路由）*
