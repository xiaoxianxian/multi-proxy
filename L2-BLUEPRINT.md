...
| **P2** | 健康监控增强（告警+成本分析） | 2-3 天 | Agent Registry | 告警服务 + 成本报告 | ⚠️ **告警内核已落地 `l2/alert.js`**：4 规则引擎（全网故障 critical / 单点 unhealthy warning / 错误高频 / 成本超预算）+ cooldown 去重防风暴 + 可插拔 sink（log 默认收集/notify observe）+ `PROXY_HEALTH_ALERT` 门控（默认 observe 不真发）+ 非侵入（不写 providers.json/不触路由/不落盘），信号注入式消费 provider-health.listIsolated()/error-patterns（不硬绑 manager lib，与 l2/ 其它内核同构）；demo 13/13、jest 13/13、全量 596/596。**成本分析缺**——成本规则已就位但 `forward.js` 暂无 token usage 埋点（喂信号即触发），需先定 A(usage 埋点,改热路径) vs B(余额趋势,零热路径)，列独立增量 |
