...
|--------|------|--------|------|--------|------|
| **P0** | 插件运行时框架 | 2-3 天 | 无 | 插件加载器 + 接口契约 + 示例插件（含 zg 成本优化样例） | 未开始 |
| **P0** | **Agent Adapter 协议** | 2-3 天 | 插件运行时 | 任务接收/结果回传/能力声明/健康检查 契约 + Codex/Hermes/h3web/AIGC 各 1 样例 | 未开始 |
| **P0** | Agent Registry | 1-2 天 | 插件运行时 | Agent Profile CRUD API + 能力标签注册 | 未开始 |
| **P1** | 记忆服务（公共+个性+环境变量注入） | 2 天 | Agent Registry | 记忆合并 + 规范数据格式 + 方案适配器 + 环境变量注入 | 未开始 |
| **P1** | 技能服务（含互通性） | 2 天 | Agent Registry | Skill CRUD + 版本 + 规范格式适配 | 未开始 |
| **P2** | 编排引擎（任务拆解+路由+执行+聚合） | 5-7 天 | Agent Registry + 记忆服务 + Adapter | 编排 API + 5 用例（含 h3web 视频工作流） | 未开始 |
| **P2** | 健康监控增强（告警+成本分析） | 2-3 天 | Agent Registry | 告警服务 + 成本报告 | 未开始 |
| **P3** | 文档 + 案例 + 性能测试 | 1 周 | 全部 | README / ARCHITECTURE / 3 案例 / 测试报告 | 未开始 |
...
*最后更新：2026-09-11（v2：整合老板 4 点补充 + 3 个卡点决议；Agent Adapter 协议升为 P0 核心卖点）*